        ...
        <tbody>
          ...
          >
            ...
            <td
              ...
              :class="[hoveredRow === rowIndex && cellIndex === 1 ? 'text-common_color_3' : 'text-gray-900']"
              >
              <span
                v-if="cellIndex === 1"
              ...
              // 新增条件：当rowIndex=3时根据内容设置颜色
                {
                  'text-common_color_3 font-semibold': cellIndex === 3 && row[header.name] === 'completed',
                  'text-common_color_1 font-semibold': cellIndex === 3 && row[header.name] === 'failed',
                  'text-common_color_2 font-semibold': cellIndex === 3 && row[header.name] === 'ongoing'
                }">
                {{ row[header.name] }}
              </span>
            ...
            <td class="px-6 py-4 border-b border-gray-300 text-sm leading-5 relative">
              <div v-show="hoveredRow === rowIndex" class="flex flex-row items-center space-x-2 absolute right-2 top-1/2 transform -translate-y-1/2">
                <button
                  @click.stop="handleRetry(row)"
                  class=" w-12 h-8 bg-common_color_2 text-white rounded px-3 py-1 text-xs">
                  重试
                </button>
                <button
                  @click.stop="handleDelete(row)"
                  class=" w-12 h-8 bg-common_color_1 text-white rounded px-3 py-1 text-xs">
                  删除
                </button>
              </div>
            </td>
